...
describe("LaunchTable", () => {
  ...
  it("should fetch and display launch data", async () => {
    ...
    await waitFor(() => {
      mockLaunches.forEach((launch) => {
        ...
        expect(
          screen.getByText(new Date(launch.date_utc).toLocaleDateString())
        ).toBeInTheDocument();
  ...
  it("should display an error message when fetching launches fails", async () => {
    (fetchLaunches as jest.Mock).mockRejectedValue(
      new Error("Failed to fetch launches")
    );
